...
| W2.1 | Root-Test-Triage | k2 | **done** | `317ce48` | `spec/tests_phase_b_selection.md` (40 Root-`.moos`) |
...
| W2.3 | CI-/Pfad-Schutz | k2 | **done** | (Grep-One-Liner) | `grep -rn "test_.*\.moos" .github/ compiler/tests/run_all.sh docs/` — 0 Root-Pfade in CI. |
| W3.1 | Beispiele WEB | k3 | **done** | 3 Commits bis `1e8cec0` | 5 Dateien → `beispiele/domain/web/`. CI 0 Brüche. |
| W3.2 | Beispiele DB | k3 | **done** | 3 Commits bis `bccdbd5` | 8 Dateien → `beispiele/domain/db/`. Alte `chat_server.moos` bleibt als Legacy-Referenz. |
...
- Python-Parser-Endlosschleife auf `beispiele/showcase.moos` (aus `tools/fuzz_differential.py` E3-Lauf).
