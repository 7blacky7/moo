...
raum_kamera(win, 0, 0, 5, 0, 0, 0)

setze winkel auf 0.0
setze kamera_z auf 5.0
...
solange raum_offen(win):
    ...
    # Kamera-Grenzen
    wenn kamera_z < 2.0:
        setze kamera_z auf 2.0
    wenn kamera_z > 15.0:
        setze kamera_z auf 15.0

    raum_kamera(win, 0, 0, kamera_z, 0, 0, 0)
    ...
    raum_würfel(win, 0, 0, 0, 0.5, "gruen")
    raum_pop(win)
    ...
    raum_würfel(win, 0, 0, 0, 0.5, "blau")
    raum_pop(win)

    # Boden (grau, flach, statisch)
    raum_push(win)
    raum_verschiebe(win, 0, -2.0, 0)
    raum_würfel(win, 0, 0, 0, 4.0, "grau")
    raum_pop(win)
