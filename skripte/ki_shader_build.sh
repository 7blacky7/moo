...
OUT=moo_ki_gpu_shaders.h
{
  ...
  echo " * Quellen: .comp-Dateien unter runtime/shader_ki (GPU2, Plan-014). */"
} > "$OUT"
for s in matmul matmul_naiv elementwise reduce unary_fwd unary_bw transpose copy grad_accum opt_sgd opt_adam softmax_fwd softmax_bw ce_fwd ce_bw norm_fwd norm_bw reduce_axis broadcast reduce_max_bw gather scatter_add rope head_slice im2col col2im pool_fwd pool_bw; do
