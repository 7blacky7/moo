...
main(void)
{
...
#if defined(MOO_HAS_VULKAN)
    MooCompGpuExecutionReport first_report;
    ...
    result = moo_comp_effects_gpu_vulkan_execute_readback(
        &backend, &graph, MOO_COMP_GPU_QUALITY_Q0_CONTENT,
        &source, &output, &report);
#if defined(MOO_HAS_VULKAN)
    if (result == MOO_COMP_UNSUPPORTED) {
        puts("SKIP: no attested hardware Vulkan device");
        return 77;
    }
    CHECK(result == MOO_COMP_OK, "hardware Vulkan Q0 succeeds");
    CHECK(moo_ki_gpu_device_info(&device) &&
        (device.device_kind == MOO_KI_GPU_DEVICE_DISCRETE ||
         device.device_kind == MOO_KI_GPU_DEVICE_INTEGRATED),
        "selected Vulkan device is hardware");
    CHECK(report.requested_quality == MOO_COMP_GPU_QUALITY_Q0_CONTENT &&
        ...
        "report proves requested/executed hardware Q0");
    CHECK(report.vendor_id == device.vendor_id &&
        report.device_id == device.device_id &&
        report.api_level == device.api_version,
        "report binds selected Vulkan identity");
    CHECK(report.graph_hash == graph.semantic_hash &&
        report.readback_hash != 0u, "report binds graph and pixel hashes");
    ...
    CHECK(memcmp(source_pixels, output.pixels, ROW * H) == 0,
        "Vulkan readback pixels exactly match CPU source");
    CHECK(memcmp(guarded_output, before, GUARD) == 0 &&
        memcmp(guarded_output + GUARD + ROW * H,
            before + GUARD + ROW * H, GUARD) == 0,
        "readback preserves guards");
    moo_ki_gpu_telemetrie(&telemetry);
    CHECK(telemetry.uploads == UINT64_C(1) &&
        telemetry.submits == UINT64_C(1) &&
        telemetry.downloads == UINT64_C(1) &&
        telemetry.cpu_fallbacks == UINT64_C(0),
        "telemetry proves one upload/submit/download and no fallback");

    printf("P016 VULKAN GPU Q0 PROOF: kind=%u vendor=%08x device=%08x api=%08x graph=%016llx readback=%016llx submits=%llu\n",
        (unsigned)device.device_kind, (unsigned)report.vendor_id,
        (unsigned)report.device_id, (unsigned)report.api_level,
        (unsigned long long)report.graph_hash,
        (unsigned long long)report.readback_hash,
        (unsigned long long)telemetry.submits);

    first_report = report;
    memset(output.pixels, 0x33, ROW * H);
    result = moo_comp_effects_gpu_vulkan_execute_readback(
        &backend, &graph, MOO_COMP_GPU_QUALITY_Q0_CONTENT,
        &source, &output, &report);
    CHECK(result == MOO_COMP_OK && report.submission.serial == 2u,
        "second hardware submission completes");
    CHECK(report.readback_hash == first_report.readback_hash &&
        memcmp(source_pixels, output.pixels, ROW * H) == 0,
        "second readback is byte/hash deterministic");

    memcpy(before, guarded_output, sizeof before);
    report_before = report;
    for (uint32_t quality =
             (uint32_t)MOO_COMP_GPU_QUALITY_Q1_GEOMETRY;
         quality <= (uint32_t)MOO_COMP_GPU_QUALITY_Q3_BACKDROP;
         ++quality) {
        result = moo_comp_effects_gpu_vulkan_execute_readback(
            &backend, &graph, (MooCompGpuQuality)quality,
            &source, &output, &report);
        CHECK(result == MOO_COMP_UNSUPPORTED,
            "Q1-Q3 level is explicit unsupported, never a Q0 claim");
        CHECK(memcmp(guarded_output, before, sizeof before) == 0 &&
            memcmp(&report, &report_before, sizeof report) == 0,
            "unsupported Q1-Q3 changes no output/report");
    }

    CHECK(moo_comp_effects_gpu_backend_mark_lost(&backend) == MOO_COMP_OK,
        "backend can be marked lost");
    result = moo_comp_effects_gpu_vulkan_execute_readback(
        &backend, &graph, MOO_COMP_GPU_QUALITY_Q0_CONTENT,
        &source, &output, &report);
    CHECK(result == MOO_COMP_UNSUPPORTED,
        "lost backend rejects Vulkan execution");
#else
    CHECK(result == MOO_COMP_UNSUPPORTED,
        "non-Vulkan build is explicit unsupported");
    CHECK(memcmp(guarded_output, before, sizeof before) == 0 &&
        memcmp(&report, &report_before, sizeof report) == 0,
        "non-Vulkan path changes no output/report");
#endif

    if (failures != 0) {
        printf("P016 VULKAN GPU Q0 FAIL: %d/%u\n", failures, checks);
        return 1;
    }
    printf("P016 VULKAN GPU Q0 GREEN: %u\n", checks);
    return 0;
}
