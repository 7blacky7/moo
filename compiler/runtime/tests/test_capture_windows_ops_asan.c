...
static MooWinCaptureResult wait_result=MOO_WIN_OK;
...
static MooWinCaptureResult enumerate(MooWinCameraInfo*out,int32_t cap,int32_t*total,char*e,size_t ec){
...
static MooWinCaptureResult cam_open(const char*id,int32_t w,int32_t h,double fps,bool exact,
 void**s,int32_t*aw,int32_t*ah,double*af,int32_t*b,char*e,size_t ec){
 (void)id;(void)exact;(void)e;(void)ec;*s=(void*)0x11;*aw=w;*ah=h;*af=fps;*b=2;cam_step=0;return MOO_WIN_OK;
}
static MooWinCaptureResult cam_wait(void*s,int32_t t,char*e,size_t ec){(void)s;(void)t;(void)e;(void)ec;return wait_result;}
static MooWinCaptureResult cam_next(void*s,MooWinFramePacket*p,char*e,size_t ec){
 (void)s;(void)e;(void)ec;if(cam_step>=2)return MOO_WIN_EMPTY;
 p->width=2;p->height=1;p->stride=8;p->bytes=8;p->bgra=malloc(8);
 uint8_t v=(uint8_t)(10+cam_step*20);uint8_t q[8]={v,2,3,255,4,5,6,255};memcpy(p->bgra,q,8);cam_step++;return MOO_WIN_OK;
}
static void cam_release(MooWinFramePacket*p){releases++;free(p->bgra);memset(p,0,sizeof(*p));}
static void cam_close(void*s){(void)s;cam_closes++;}
static MooWinCaptureResult mic_open(const char*id,int32_t r,int32_t c,void**s,int32_t*ar,int32_t*ac,
 int32_t*p,int32_t*b,char*e,size_t ec){(void)id;(void)e;(void)ec;*s=(void*)0x22;*ar=r;*ac=c;*p=2;*b=8;audio_step=0;return MOO_WIN_OK;}
static MooWinCaptureResult mic_wait(void*s,int32_t t,char*e,size_t ec){
 (void)s;(void)t;(void)e;(void)ec;if(wait_result==MOO_WIN_RECOVERABLE&&recover_count==0)return MOO_WIN_RECOVERABLE;return wait_result==MOO_WIN_RECOVERABLE?MOO_WIN_OK:wait_result;
}
static const float a0[]={1,3,2,4,5,7,6,8};
static MooWinCaptureResult mic_next(void*s,MooWinAudioPacket*p,char*e,size_t ec){
 (void)s;(void)e;(void)ec;if(audio_step++)return MOO_WIN_EMPTY;
 p->samples=a0;p->frames=4;p->channels=2;p->token=(void*)a0;return MOO_WIN_OK;
}
static void mic_release(MooWinAudioPacket*p){releases++;p->token=NULL;}
static MooWinCaptureResult recover(void*s,char*e,size_t ec){(void)s;(void)e;(void)ec;recover_count++;return MOO_WIN_OK;}
static void mic_close(void*s){(void)s;mic_closes++;}
static const MooCaptureWindowsOps fake={clock_ms,startup,shutdown_,enumerate,cam_open,cam_wait,cam_next,
 cam_release,cam_close,mic_open,mic_wait,mic_next,mic_release,recover,mic_close};
const MooCaptureWindowsOps* moo_capture_windows_system_ops(void){return &fake;}
static void reset(void){moo_error_flag=0;wait_result=MOO_WIN_OK;now_ms=1000;releases=0;recover_count=0;}

int main(void){
 moo_capture_windows_set_ops_for_tests(&fake);reset();
 ...
 wait_result=MOO_WIN_TIMEOUT;MooValue no=moo_capture_camera_frame_native(&c,0);CK(moo_error_flag&&no.tag==MOO_NONE,"timeout=0 definiert");moo_capture_camera_close_native(&c);CK(cam_closes>0&&c.backend==NULL,"Kamera Cleanup idempotent");
 ...
 wait_result=MOO_WIN_RECOVERABLE;audio_step=0;ar=moo_capture_microphone_read_native(&m,1,50);
