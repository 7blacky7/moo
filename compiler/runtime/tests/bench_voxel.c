 ...
 *
 * Plan-006 P006-R0: Die Host-Stubs (moo_alloc, MooValue-Ctors, Dict-Stub,
 * headless 3D-API, peak_rss_kb, now_ms, N(), externe Voxel-Decls) wurden in
 * bench_voxel_harness.h ausgelagert und werden mit bench_voxel_ram.c geteilt
 * (Reuse statt Duplikat). Die RAM-Kategorien-Messung lebt in bench_voxel_ram.c;
 * hier bleiben die Perf-Timings (a)-(d).
 *
 ...
 *
 * Hier wird das ECHTE moo_noise.c gelinkt (Worldgen-Determinismus). Die 3D-API
 * ist headless gestubbt (kein GPU). Der GPU-Upload (moo_voxel_aktualisieren)
 * braucht ein aktives Backend; fuer das Remesh-CPU-Timing nutzen wir den
 * synchronen Upload-Pfad mit aktivem Stub-Backend, sodass die GESAMTE
 * Greedy+AO-CPU-Arbeit + Buffer-Aufbau gemessen wird.
 */
